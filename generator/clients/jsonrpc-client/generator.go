...
import (
	...
	"gitlab.forceup.in/qingyun/polaris/utils/_rpc"
)
...
func NewClient(addr string, opts ..._rpc.Option) (c *Client, err error) {
	c = &Client{}
	opts = append(opts, _rpc.WithOut(
		&c.BillAPIStruct.Internal,
		&c.CustomerAPIStruct.Internal,
		&c.MinerAPIStruct.Internal,
	))
	c.closer, err = _rpc.NewClient(addr, BillBoxNamespace, opts...)
	if err != nil {
		return
	}
	return
...
func Generate(path, pkg, outpkg, outfile string) error {
	...
	if ap == nil {
		return fmt.Errorf("package: %s not found in %s", pkg, path)
	}
	...
	err = doTemplate(w, m, `// Code generated by github.com/gozelle/mix with "mix generate client". DO NOT EDIT.
...
`)
	...
	r, err := imports.Process("", w.Bytes(), &imports.Options{})
	if err != nil {
		return err
	}
